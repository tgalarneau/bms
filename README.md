...
The JBD BMS uses either serial or bluetooth to access its data and this is for the bluetooth interface. The way this was implemented is not standard as in sending read requests or turning on notifications to receive data.
...
For this, bms.py alone is required. Just discover your JBD BLE address. I use hcitool lescan to find this or the Xiaoxiang app also shows this at startup. The bms.py program is initiated with a (-b) device BLE address, (-i) collection interval and (-m) monitor name.
